...
export default function CartList() {
  ...
  //     "best_seller": true
  //   }
  // ] // fake list, to iterate over and map the cartCards - at first

  const { cartList, state } = useCart(); // gewährleistet Zugriff auf total und cartList und die funktionen add,remove,clearCart
  const products = cartList;


  // Handhabung des Toggeln des Checkout Window (Komponente)
  const [showCheckout, setShowCheckout] = useState(false); 
    return (
        <>
          <section>
            <p className="text-2xl text-center font-semibold dark:text-slate-100 my-10 underline underline-offset-8">
              My Cart ({cartList.length}) {/* useContex case */}
            </p>
          </section>
          ...
          <section className="max-w-4xl m-auto">
            <div className="flex flex-col p-2 border-b dark:border-slate-700 text-lg dark:text-slate-100">
              <p className="flex justify-between my-2">
                ...
                <span>${state.total}</span> {/* useContex case */}
              </p>
            </div>
